...
Component* Frame::eventHandler(MyEvent e)
{
	...
	if (e.isLButtonUp() || e.isRButtonUp()) {
		//end_ = e.getPos();
		Component* b = findMenu(e.getPos());
		if (b) {
			b->onClick(e);
			return b;
		}
...
void Frame::add(Component* b) {
	myMenulist.push_back(b);
	//b->setParent(this);
}

Component* Frame::findMenu(MyPoint p) {
	for (auto b : myMenulist) {
		if (b->inside(p)) {
			return b;
		}
	}
...
void Frame::setFigType(Component* b) {
	return;
}

void Frame::setCalcType(Component* c) {
	return;
}

void Frame::setMenuType(Component* b) {
	return;
}